...
class Validator {
    ...
    private readonly AllowedValidator = () => {
        ...
        if (!allowed[key])
            throw new Error(`${key} in allowed settings is not defined`);
    ...
    public readonly init = (): Settings => {
        ...
        if (!settings[key]) {
            console.log(`Value at key: "${key}" is not allowed, you can use:\r\n`, Object.keys(settings));
            throw new Error(`Value at key: "${key}" is not allowed`);
        }

        if (!value) {
            console.log(`Value at key: "${key}" is not defined\r\nThis value can be:\r\n`, settings[key]);
            if (Object.keys(allowed).includes(key)) console.log('Or other allowed values:\r\n', allowed[key]);
